...
DEFAULT_CNAME_MAP = {
    "EMERGE-PEC": "#ff78aa",
    "EMERGE-DIEL": "#10ad10",
    "EMERGE-COPPER": "#b87333",
    "EMERGE-AIR": "#e5f8fc",
    "EMERGE-SELECT": "#64a7ff",
    "EMERGE-TEXT": "#000000",
    "red": "#ef0e2f",
    "green": "#4bf701",
    "blue": "#005eff",
...
def dict_from_default(input_dict: dict, default_dict: dict) -> dict:
    return {**default_dict, **input_dict}
...
class EMergeTheme:
    def __init__(self):
        ...
        self.render_diffuse = 0.6
        self.render_specular = 0.8
        ...
        self.colormaps: dict[str, tuple[list[str], list[float]]] = {
            "amplitude": (
                ["#1F0061", "#4218c0", "#2849db", "#ff007b", "#ff7c51"],
                (0.0, 0.15, 0.3, 0.7, 0.95),
            ),
            "wave": (
                [
                    "#4ab9ff",
        ...
        # Color name dict:
        self.color_name_map: dict[str, str] = dict()
        self.opacity_codes: dict[str, float] = dict()
        self.default_opacity: float = 0.5
        self.default_color: str = "#9c9292"
        ...
        self.surf_kwargs = dict()
        self.cloud_kwargs = dict()
        self.quiver_kwargs = dict()
        self.contour_kwargs = dict()
        self.farfield_3d_kwarg: dict = dict()
    ...
    def _init(self):
        ...
        self.color_name_map = {**DEFAULT_CNAME_MAP, **self.color_name_map}
        self.opacity_codes = {**DEFAULT_OPACITY, **self.opacity_codes}
    ...
    def parse_opacity(self, value: str | float) -> float:
        ...
        if isinstance(value, str):
            return self.opacity_codes.get(value, self.default_opacity)
        else:
    ...
    def parse_color_name(self, colorname: str) -> str:
        """Converts a color name from the library

        Args:
            colorname (str): The name of the color or hex code

        Returns:
            str: _description_
        """
        return self.color_name_map.get(colorname, self.default_color)
    ...
    def _apply_color_blend(
        ...
        if blending_mode == "normal":
            ...
            def soft_light_channel(bc, mc):
                return (1 - 2 * (alpha * mc + (1 - alpha))) * bc * bc + 2 * (
                    alpha * mc + (1 - alpha)
                ) * bc
...
class PVDisplaySettings:
    def __init__(self):
        ...
        self.light_angle: tuple[float, float] = (20.0, -20.0)
        self.z_boost: float = 0.0
        self.depth_peeling: bool = True
        self.gauss_smoothing: bool = False
